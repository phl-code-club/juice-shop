...
export const accessControlChallenges = () => ({ url }: Request, res: Response, next: NextFunction) => {
  ...
  challengeUtils.solveIf(challenges.securityPolicyChallenge, () => { return utils.endsWith(url.toLowerCase(), '/security.txt') })
  ...
  void osaft.reload().then(() => {
    ...
    if (urlForProductTamperingChallenge) {
      if (!utils.contains(osaft.description, `${urlForProductTamperingChallenge}`)) {
        if (utils.contains(osaft.description, `<a href="${config.get<string>('challenges.overwriteUrlForProductTamperingChallenge')}"`)) {
